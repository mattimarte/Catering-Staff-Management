...
public class KitchenTaskManager {
    ...
    public KitchenSheet generateSummarySheet(Event event, Service service) throws UseCaseLogicException {
        ...
        KitchenSheet newKitchenSheet = new KitchenSheet(service, user);

        this.setCurrentKitchenSheet(newKitchenSheet);
        this.notifySheetGenerated(newKitchenSheet);
        return newKitchenSheet;
    }

    public ArrayList<KitchenSheet> loadAllSumSheets() {
        return KitchenSheet.loadAllSumSheets();
    }

    public KitchenSheet openSumSheet(KitchenSheet ss) throws UseCaseLogicException, SummarySheetException {
        User user = CatERing.getInstance().getUserManager().getCurrentUser();
        if (!user.isChef())
            throw new UseCaseLogicException();
        if (!ss.isOwner(user))
            throw new SummarySheetException("User: " + user.getUserName() + " is not owner of the SummarySheet");
        setCurrentKitchenSheet(ss);
        return ss;
    }

    public void addKitchenTask(KitchenTask t) {
        KitchenTask added = currentKitchenSheet.addTask(t);
        notifyTaskAdded(added);
    }

    public void moveTask(KitchenTask t, int pos) throws UseCaseLogicException {
        if (currentKitchenSheet == null || currentKitchenSheet.getTaskPosition(t) < 0)
            throw new UseCaseLogicException();
        if (pos < 0 || pos >= currentKitchenSheet.getTaskListSize())
            throw new IllegalArgumentException();
        this.currentKitchenSheet.moveTask(t, pos);
    ...
    public void addTaskInformation(KitchenTask task, int quantity, int portions, long minutes)
        ...
        if (currentKitchenSheet == null)
            throw new UseCaseLogicException();
        if (currentKitchenSheet.getTaskPosition(task) < 0)
        ...
        KitchenTask t = currentKitchenSheet.addTaskInformation(task, quantity, portions, minutes);
    ...
    public Assignment assignTask(KitchenTask t, Shift s, User cook) throws UseCaseLogicException {
        ...
        Assignment a = currentKitchenSheet.addAssignment(t, s, cook);
    ...
    public void modifyAssignment(Assignment ass, Shift shift, User cook)
        ...
        if (cook == null || CatERing.getInstance().getShiftManager().isAvailable(cook, shift))
            a = currentKitchenSheet.modifyAssignment(ass, shift, cook);
        else
            throw new UseCaseLogicException();
     ...
     */
    public KitchenSheet getCurrentSummarySheet() {
        return currentKitchenSheet;
    }

    public void setTaskReady(KitchenTask t) throws UseCaseLogicException {
        KitchenTask task = currentKitchenSheet.setTaskReady(t);
        notifyTaskChanged(task);
    }

    public void deleteAssignment(Assignment a) throws UseCaseLogicException {
        Assignment ass = currentKitchenSheet.deleteAssignment(a);
        notifyAssignmentDeleted(ass);
    }

    private void setCurrentKitchenSheet(KitchenSheet kitchenSheet) {
        this.currentKitchenSheet = kitchenSheet;
    }
    ...
    private void notifyAssignmentAdded(Assignment assignment) {
        for (KitchenTaskEventReceiver er : eventReceivers) {
            er.updateAssignmentAdded(currentKitchenSheet, assignment);
        }
    }

    private void notifyTaskListSorted() {
        for (KitchenTaskEventReceiver er : eventReceivers) {
            er.updateTaskListSorted(currentKitchenSheet);
        }
    }

    private void notifyTaskAdded(KitchenTask added) {
        for (KitchenTaskEventReceiver er : eventReceivers) {
            er.updateTaskAdded(currentKitchenSheet, added);
        }
    }

    private void notifySheetGenerated(KitchenSheet summarySheet) {
        for (KitchenTaskEventReceiver er : eventReceivers) {
